...
public class Subtitles extends AutomaticsTestBase{
	...
	@TestDetails(testUID = "TC-RDKV-STB-1014")   //Change testUID
	public void testVerifySubtitles (Dut device) {  // Change method
		// Variables declaration starts
		boolean status = false;
		String testId = "TC-RDKV-STB-014"; //Change manual ID
		...
		LOGGER.info("#######################################################################################");
		LOGGER.info("STARTING TEST CASE: TC-RDKV-STB-1014");
		LOGGER.info("TEST DESCRIPTION: Verify the subtitles icon of the screen matches with the reference image");
		LOGGER.info("TEST STEPS : ");
		LOGGER.info("1. Verify the Peacock subtitles icon with reference image");
		LOGGER.info("#######################################################################################");
...
//		LOGGER.info("The tapenv data: " +tapEnv);
				try {
					stepNum = "S1";
					errorMessage = "The Peacock Mystuff screen and the current screen doesn't match";
					LOGGER.info("*****************************************************************************************");
					LOGGER.info("STEP 1: DESCRIPTION : Verify the subtitles icon of the screen matches with the reference image.");
					LOGGER.info("STEP 1: ACTION : ACTION: Capture the live screen subtitles icon and compare it with actual image.");
					LOGGER.info("STEP 1: EXPECTED : The live screen subtitles icon should match with reference image.");
					LOGGER.info("*****************************************************************************************");
					
					reference_image = ImageIO.read(new File(ReferenceImageConstants.SUBTITLE_ICON_REFERENCE_IMG)); //change
					
					live_Image = ImageIO.read(new File(ReferenceImageConstants.SUBTITLE_ICON_LIVE_IMG)); //Change
